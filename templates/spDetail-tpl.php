...
if (!is_null($spName) && !empty($spName)) {
    $this->data['header'] = $this->t('{proxystatistics:Proxystatistics:templates/spDetail_header_name}') . $spName;
} else {
    $this->data['header'] = $this->t('{proxystatistics:Proxystatistics:templates/spDetail_header_identifier}') . $spIdentifier;
}
...
?>
    <script type="text/javascript">
        ...
        function drawUsedIdpsTable() {
            ...
            data.addRows([<?php DatabaseCommand::getAccessCountForServicePerIdentityProviders($lastDays, $spIdentifier)?>]);
...
<body>
    <div class="go-to-stats-btn">
        <a href="./" class="btn btn-md btn-default"><span class="glyphicon glyphicon-home"></span>  <?php echo $this->t('{proxystatistics:Proxystatistics:btn_label_back_to_stats}'); ?></a>
    </div>
